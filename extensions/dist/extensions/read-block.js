...
// extensions/read-block.ts
var readBlockSchema = Type.Object({
...
function renderReadBlockResult(result, { expanded, isPartial }, theme) {
  if (isPartial)
    return new Text(theme.fg("warning", "Reading block..."), 0, 0);
  const details = result.details;
  if (!details)
    return new Text(theme.fg("accent", "Read block"), 0, 0);
...
function readBlockExtension(pi) {
  pi.registerTool({
    name: "read_block",
    label: "read_block",
    description: "Read one semantic block around a specific line or symbol: markdown section, brace block, CSS rule, or indentation block. This is for targeted inspection, not sequential file scanning.",
    ...
    renderResult(result, options, theme, context) {
      if (options.expanded || !canGroupTool(context))
        return renderReadBlockResult(result, options, theme);
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (capped.truncated)
        header.push(`Truncated: showing L${capped.start + 1}-L${capped.end + 1} of L${capped.originalStart + 1}-L${capped.originalEnd + 1}`);
