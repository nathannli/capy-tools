...
var CHANGE_CONTEXT = "@@ ";
var EMPTY_CHANGE_CONTEXT = "@@";
var applyPatchSchema = Type.Object({
...
function renderApplyPatchResult(result, { expanded, isPartial }, theme) {
  if (isPartial)
...
function unwrapLenientPatch(raw) {
...
function parsePatch(raw) {
  ...
  if (firstBodyLine?.startsWith(ENVIRONMENT_ID)) {
    ...
    if (!environmentId)
      throw new Error("invalid patch: apply_patch environment_id cannot be empty");
    body = body.slice(1);
  }
  const hunks = [];
  ...
  if (hunks.length === 0)
    throw new Error("invalid patch: No files were modified.");
...
`;
  ...
  throw new Error(`invalid hunk at line ${lineNumber}: '${firstLine}' is not a valid hunk header. Valid hunk headers: '${ADD_FILE}{path}', '${DELETE_FILE}{path}', '${UPDATE_FILE}{path}'`);
}
function parseUpdateChunk(lines, lineNumber, allowMissingContext) {
  if (lines.length === 0)
  ...
  for (const line of lines.slice(startIndex)) {
    ...
    if (first === undefined) {
      ...
      if (parsedLines === 0) {
        throw new Error(`invalid hunk at line ${lineNumber + 1}: Unexpected line found in update hunk: '${line}'. Every line should start with ' ' (context line), '+' (added line), or '-' (removed line)`);
      }
      break;
    }
    parsedLines += 1;
...
function displayPath(cwd, absolutePath, originalPath) {
...
function seekSequence(lines, pattern, start, eof) {
  ...
  const passes = [
    ...
    (line, pat) => normalizeForFuzzyMatch(line) === normalizeForFuzzyMatch(pat)
  ];
...
async function ensureNotDirectory(path, action) {
  const stats = await lstat(path);
  if (stats.isDirectory())
    throw new Error(`Failed to ${action} ${path}: path is a directory`);
}
async function deriveNewContents(path, chunks) {
...
async function applyHunks(hunks, cwd) {
  const changes = [];
...
function summarize(details) {
  ...
  if (details.partialFailure)
    lines.push("Partial failure: some earlier changes were already written.");
...
function detailsFromChanges(cwd, changes, error) {
...
function applyPatchExtension(pi) {
  pi.registerTool({
    name: "apply_patch",
    label: "apply_patch",
...
export {
  applyPatchExtension as default
};
