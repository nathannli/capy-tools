...
import { basename as basename2, extname as extname2, resolve } from "node:path";

// extensions/block-utils.ts
...
function findAnchorLine(lines, symbol, line) {
  if (line !== undefined) {
    const idx = Math.floor(line) - 1;
    if (idx < 0 || idx >= lines.length)
      throw new Error(`line ${line} is outside file range 1-${lines.length}`);
    return { index: idx, reason: `line ${line}` };
  }
  const needle = symbol?.trim();
  if (!needle)
    throw new Error("Provide either line or symbol.");
  const word = new RegExp(`\\b${escapeRegex(needle)}\\b`);
  for (let i = 0;i < lines.length; i++) {
    if (declarationName(lines[i]) === needle)
      return { index: i, reason: `declaration '${needle}'` };
  }
  for (let i = 0;i < lines.length; i++) {
  ...
  for (let i = 0;i < lines.length; i++) {
    if (word.test(lines[i]))
      return { index: i, reason: `symbol '${needle}'` };
  }
  for (let i = 0;i < lines.length; i++) {
    if (lines[i].includes(needle))
      return { index: i, reason: `text '${needle}'` };
  }
  throw new Error(`Could not find symbol or text '${needle}'`);
}
function markdownBlock(lines, anchor) {
  let start = anchor;
  let level = 7;
...
function braceBlock(lines, start) {
  let openSeen = false;
  let balance = 0;
  for (let i = start;i < lines.length; i++) {
    ...
    if (!openSeen && i > start + 8)
      return;
  }
  return;
}
function findParentStart(lines, anchor) {
  let current = anchor;
  while (current > 0 && isBlank(lines[current]))
    current--;
  const anchorIndent = indentOf(lines[current]);
  if (looksLikeBlockStart(lines[current]))
    return current;
  for (let i = current - 1;i >= 0; i--) {
    if (isBlank(lines[i]) || isComment(lines[i]))
      continue;
    const indent = indentOf(lines[i]);
    if (indent < anchorIndent && looksLikeBlockStart(lines[i]))
      return i;
    if (indent === 0 && anchorIndent === 0)
      return current;
  }
  return current;
}
function indentationBlock(lines, anchor) {
  ...
  while (start > 0) {
    ...
    if (indentOf(prev) < baseIndent)
      break;
    if (baseIndent === 0 && indentOf(prev) === 0 && looksLikeBlockStart(lines[startCandidate]))
      break;
    start--;
  }
  ...
  for (let i = startCandidate + 1;i < lines.length; i++) {
    ...
    if (baseIndent === 0 && indent === 0 && looksLikeBlockStart(lines[startCandidate]) && i > startCandidate)
      break;
...
function windowBlock(lines, anchor, maxLines) {
...
function detectMode(filePath, requested) {
  const mode = (requested ?? "auto").toLowerCase();
  if (mode === "markdown" || mode === "indentation" || mode === "window" || mode === "auto")
    return mode;
  throw new Error("mode must be one of: auto, markdown, indentation, window");
}
function chooseBlock(filePath, lines, anchor, mode, maxLines) {
  const ext = extname(filePath).toLowerCase();
  if (mode === "markdown" || mode === "auto" && (ext === ".md" || ext === ".mdx"))
    return markdownBlock(lines, anchor);
  if (mode === "window")
    return windowBlock(lines, anchor, maxLines);
...
function capRange(range, anchor, context, maxLines, totalLines) {
  ...
  if (length <= maxLines)
    return { ...range, start: originalStart, end: originalEnd, truncated: false, originalStart, originalEnd };
...
// extensions/symbol-outline.ts
var symbolOutlineSchema = Type.Object({
...
function compactSignature(line, maxLength) {
...
function declarationKind(line) {
  ...
  if (/^(export\s+)?enum\b/.test(trimmed))
    return "enum";
  if (/^(export\s+)?const\b/.test(trimmed))
    return "const";
  if (/^(export\s+)?let\b/.test(trimmed))
    return "let";
  if (/^(export\s+)?var\b/.test(trimmed))
    return "var";
  if (/^def\b/.test(trimmed))
    return "def";
  if (/^class\b/.test(trimmed))
    return "class";
  if (/^(pub\s+)?(async\s+)?fn\b/.test(trimmed))
    return "function";
  if (/^(pub\s+)?struct\b/.test(trimmed))
    return "struct";
  if (/^(pub\s+)?enum\b/.test(trimmed))
    return "enum";
  if (/^(pub\s+)?trait\b/.test(trimmed))
    return "trait";
  if (/^(pub\s+)?impl\b/.test(trimmed))
    return "impl";
  if (/^func\b/.test(trimmed))
    return "function";
  return;
}
function shouldIncludeDeclaration(line, includeNested) {
  if (includeNested)
    return true;
  return indentOf(line) === 0;
}
function buildOutlineBlocks(filePath, displayPath, lines, mode, includeNested, maxSignatureLength) {
  ...
  for (let i = 0;i < lines.length; i++) {
    ...
    if (markdown) {
      ...
      if (!name || !shouldIncludeDeclaration(lines[i], includeNested))
        continue;
      kind = "css-rule";
    } else {
...
function renderSymbolOutlineResult(result, { expanded, isPartial }, theme) {
  if (isPartial)
    return new Text(theme.fg("warning", "Outlining symbols..."), 0, 0);
  const details = result.details;
  if (!details)
    return new Text(theme.fg("accent", "Outline file"), 0, 0);
...
function symbolOutlineExtension(pi) {
  pi.registerTool({
    name: "symbol_outline",
    label: "symbol_outline",
    description: "List readable symbols, CSS rules, or Markdown sections in a file with line anchors that can be passed directly to read_block.",
    ...
    renderResult(result, options, theme, context) {
      if (options.expanded || !canGroupTool(context))
        return renderSymbolOutlineResult(result, options, theme);
...
export {
  symbolOutlineExtension as default
};
