...
import { basename as basename2, dirname as dirname2, extname, join as join2, relative, resolve } from "node:path";
import { spawnSync } from "node:child_process";
var repoMapSchema = Type.Object({
...
var CONFIG_PATTERNS = [/^eslint\.config\./, /^prettier\.config\./, /^biome\.json/, /^vitest\.config\./, /^jest\.config\./, /^tailwind\.config\./];
var EXT_LANG = {
  ".ts": "TypeScript",
...
function renderRepoMapResult(result, { expanded, isPartial }, theme) {
  if (isPartial)
    return new Text(theme.fg("warning", "Mapping repository..."), 0, 0);
  const details = result.details;
  if (!details)
    return new Text(theme.fg("accent", "Map project"), 0, 0);
  const name = basename2(details.root || "project");
...
function splitNul(text) {
  return text ? text.split("\x00").filter(Boolean) : [];
}
function findProjectRoot(start) {
  let current = resolve(start);
  while (true) {
    if (existsSync(join2(current, ".git")) || existsSync(join2(current, "package.json")) || existsSync(join2(current, "Cargo.toml")) || existsSync(join2(current, "go.mod")) || existsSync(join2(current, "pyproject.toml"))) {
      return current;
    }
...
function getGitInfo(root) {
  ...
  if (!gitRoot)
    return { status: [], tracked: [], untracked: [], recent: [] };
...
function walkFiles(root, maxDepth, maxFiles = 5000) {
  const out = [];
  ...
  while (stack.length > 0 && out.length < maxFiles) {
    ...
    for (const entry of entries) {
      ...
      const full = join2(item.dir, entry.name);
      const rel = relative(root, full).replace(/\\/g, "/");
      if (entry.isDirectory()) {
        if (!IGNORE_DIRS.has(entry.name) && item.depth < maxDepth)
...
function fileInfosFromGit(root, paths) {
  const files = [];
  for (const rel of paths) {
    ...
    try {
      const stats = statSync(full);
      if (stats.isFile())
        files.push({ path: rel.replace(/\\/g, "/"), ext: extname(rel).toLowerCase(), size: stats.size, mtime: stats.mtimeMs });
    } catch {}
  }
  return files;
}
function summarizeLanguages(files) {
  ...
  return [...counts.entries()].sort((a, b) => b[1].files - a[1].files || b[1].bytes - a[1].bytes).slice(0, 10).map(([lang, data]) => `${lang}: ${data.files} files`);
}
function summarizeDirectories(files, depth) {
  const counts = new Map;
  for (const file of files) {
  ...
  return [...counts.entries()].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0])).slice(0, 30).map(([dir, count]) => `${dir}/ (${count})`);
}
function findImportantFiles(root, files, limit) {
  const fileSet = new Set(files.map((file) => file.path));
  const important = [];
  for (const name of IMPORTANT_ROOT_FILES) {
    if (fileSet.has(name) || existsSync(join2(root, name)))
      important.push(name);
  }
  for (const file of files) {
    const base = basename2(file.path);
    if (CONFIG_PATTERNS.some((pattern) => pattern.test(base)) && !important.includes(file.path))
  ...
  for (const file of entryHints) {
    if (!important.includes(file.path))
      important.push(file.path);
...
function readPackageSummary(root) {
  ...
  try {
    ...
    if (deps.length > 0)
      lines.push(`deps: ${deps.join(", ")}${deps.length >= 20 ? ", ..." : ""}`);
    return lines;
  } catch {
...
function repoMapExtension(pi) {
  pi.registerTool({
    name: "repo_map",
    label: "repo_map",
    description: "Generate a compact project orientation map: root, git state, manifests, language mix, important files, directories, and recent changes. This is a synthesized context summary, not a shell command wrapper.",
    ...
    renderResult(result, options, theme, context) {
      if (options.expanded || !canGroupTool(context))
        return renderRepoMapResult(result, options, theme);
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const recentByGit = git.recent.filter((file) => existsSync(join2(root, file))).slice(0, maxRecent);
      ...
      const lines = [];
      lines.push(`# repo_map: ${basename2(root) || root}`);
      lines.push("");
      lines.push(`Root: ${root}`);
      if (git.root)
        lines.push(`Git: ${git.branch ? `branch ${git.branch}` : "repository detected"}`);
      else
