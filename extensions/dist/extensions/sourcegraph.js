...
var sourcegraphSchema = Type.Object({
  query: Type.String({
    description: "Sourcegraph search query. Supports: repo:, file:, lang:, type:symbol, " + "Boolean operators (AND/OR/NOT), regex patterns. " + 'Examples: "file:.go context.WithTimeout", "lang:typescript useState type:symbol"'
  }),
  count: Type.Optional(Type.Number({ description: "Number of results to return (default 10, max 20)" })),
  context_window: Type.Optional(Type.Number({ description: "Lines of context around each match (default 10)" })),
  timeout: Type.Optional(Type.Number({ description: "Request timeout in seconds (default 30, max 120)" }))
});
var GRAPHQL_ENDPOINT = "https://sourcegraph.com/.api/graphql";
var SEARCH_QUERY = `query Search($query: String!) {
...
function renderSourcegraphResult(result, { expanded, isPartial }, theme) {
  if (isPartial)
    return new Text(theme.fg("warning", "Searching Sourcegraph..."), 0, 0);
  const details = result.details;
  if (!details)
    return new Text(theme.fg("accent", "Search Sourcegraph"), 0, 0);
  const hint = safeKeyHint2("app.tools.expand", "to expand");
  return new Text(theme.fg("accent", "Search Sourcegraph") + theme.fg("dim", `
${hint}`), 0, 0);
}
function formatResults(searchResults, contextWindow) {
  const lines = [];
  ...
  lines.push("");
  const fileMatches = searchResults.results.filter((r) => r.__typename === "FileMatch" && r.repository && r.file);
...
function sourcegraph_default(pi) {
  pi.registerTool({
    name: "sourcegraph",
    label: "sourcegraph",
    description: "Search code across public repositories using the Sourcegraph API. " + "Supports repo/file/language filters, regex, symbol search, and Boolean operators. " + "No API key required. Useful for finding reference implementations, API usage examples, " + "and patterns in open-source code.",
    ...
    renderResult(result, options, theme, context) {
      if (options.expanded || !canGroupTool(context))
        return renderSourcegraphResult(result, options, theme);
    ...
    async execute(_toolCallId, params, signal, _onUpdate, _ctx) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Sourcegraph API returned HTTP ${response.status}${body ? `: ${body.slice(0, 500)}` : ""}`);
        }
        ...
        return {
          ...
          details: {
            ...
            displayedResults: searchResults.results.filter((r) => r.__typename === "FileMatch" && r.repository && r.file).slice(0, 10).length
          }
        };
      } finally {
...
export {
  sourcegraph_default as default
};
